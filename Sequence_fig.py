...
ptm_colors = {
    ...
    's-nitrosylation': ('#1759c2', '*'),
    'Neddylation': ('#FFD700', '8'),
    'Monoubiquitination': ('#40b856', 'X'),
    'O-glcNAcylation': ('#750716', 'd'),
    'Polyubiquitination': ('#fa8907', 'H'),
    'Thiol oxidation': ('#858381', 'h')
}
        
SEQ = pd.read_csv('210Merged.csv')
df = pd.read_csv("Human_PTM.csv")
...
for uid in PROT.groups:
    ...
    for ptm, (color, marker) in ptm_colors.items():
        mod_positions = DD[(DD['PTM'] == ptm) & (~DD['SITE'].isna())]['SITE'].str[1:].astype(int).values
